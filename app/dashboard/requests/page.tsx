...
export default function RequestsPage() {
  ...
  const [profile, setProfile] = useState<{ id: string; hostel_type: 'boys' | 'girls' } | null>(null)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let query = supabase
          .from('requests')
          .select('*, requester:profiles(name, room_number, hostel_name, avatar_url)')
          .eq('status', 'open')
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-2">See what others in your hostel are looking for</p>
      </div>
      ...
      {requests.length > 0 ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mb-4">
            {filters.search || filters.category !== 'all' || filters.urgency !== 'all'
